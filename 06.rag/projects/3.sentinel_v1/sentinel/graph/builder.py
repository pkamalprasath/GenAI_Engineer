...
def build_graph(session_factory=None) -> Any:
    ...
    graph.add_conditional_edges(
        ...
        route_after_discovery,
        {
            "investigate": "investigation",      # Main path
            "hitl": "hitl_review",               # Discovery uncertain
            "no_cases": "no_cases",              # Nothing found
        },
    )

    # ── Parallel fan-out: discovery → [investigation, legal, bias] ─────────────
    # All three start after discovery completes. LangGraph runs them concurrently.
    graph.add_edge("discovery", "legal_analysis")
    graph.add_edge("discovery", "bias_detection")

    # ── Fan-in: all three feed evidence_assembly ───────────────────────────────
    ...
    logger.info('{"event":"graph_compiled","nodes":9}')
    return graph
