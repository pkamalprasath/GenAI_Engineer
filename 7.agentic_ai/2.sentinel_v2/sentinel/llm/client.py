...
# ── Phase 2C: ToolNode Support ────────────────────────────────────────────────


def get_bound_llm(tier: str, tools: list):
    """
    Return a LangChain ChatOpenAI instance with tools bound.

    This is used ONLY by legal_agent for LangGraph ToolNode architecture.
    It is NOT a replacement for chat() — it returns a LangChain chat model,
    not an LLMResponse. The LLM can emit tool_calls which ToolNode then executes.

    Args:
        tier: Model tier from models.yaml (e.g., "reasoning", "synthesis")
        tools: List of @tool-decorated callables from sentinel/tools/

    Returns:
        ChatOpenAI instance with .bind_tools(tools) applied

    Raises:
        ValueError: If tier is not configured for OpenAI provider
    """
    ...
    if cfg.provider != "openai":
        raise ValueError(
            f"get_bound_llm() only supports OpenAI provider. "
            f"Tier '{tier}' is configured for '{cfg.provider}'. "
            f"Please switch to openai in models.yaml or use a different tier."
        )
    ...
    if not api_key:
        raise RuntimeError(
            "OPENAI_API_KEY not set. Add it to .env or export it as env var."
        )
...
def get_tier_for_agent(agent_name: str) -> str:
    """
    Look up the model tier for a named agent from models.yaml agent_models mapping.

    This enables per-agent model assignment (e.g., discovery_agent uses "classification",
    legal_agent uses "reasoning") without hardcoding tier strings in agent code.

    Args:
        agent_name: Name of the agent (e.g., "legal_agent", "discovery_agent")

    Returns:
        Model tier string (e.g., "reasoning", "classification", "synthesis")
        Falls back to "reasoning" if agent_name is not in the map.
    """
    agent_models = models_cfg.get("agent_models", {})
    tier = agent_models.get(agent_name, "reasoning")
    return tier
