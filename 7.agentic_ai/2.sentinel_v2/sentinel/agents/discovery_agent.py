...
async def run(state: InvestigationState, session: AsyncSession) -> dict:
    ...
    with langfuse_tracer.trace_agent_node("discovery_agent", inv_id, tenant_id):
        try:
            log_agent_input(
                "discovery_agent", inv_id,
                ["query", "date_range", "domain", "applicant_data"],
                state
            )

            # DEBUG: Log what's being passed
            date_range = state.get("date_range", {})
            logger.info(f"[DEBUG DISCOVERY] date_range={date_range}")

            candidates = await _fetch_candidate_cases(
                session, tenant_id, date_range
            )

            logger.info(
                f"[DISCOVERY] Fetched {len(candidates) if candidates else 0} candidate cases",
                extra={"candidates_count": len(candidates) if candidates else 0}
            )

            if not candidates:
                logger.info(
                    f"[DISCOVERY] No database candidates found. Checking applicant_data...",
            ...
            log_agent_output("discovery_agent", inv_id, output)
            return output

        except Exception as exc:
            log_agent_exception("discovery_agent", inv_id, exc)
            log_error(
