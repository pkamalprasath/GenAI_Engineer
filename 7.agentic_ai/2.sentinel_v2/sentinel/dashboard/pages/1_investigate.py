...
with left_col:
    with st.form("inv_form", clear_on_submit=False):
        ...
        st.markdown('<div style="margin:16px 0 8px; font-size:13px; font-weight:700; color:#475569;">Applicant Information (Optional)</div>', unsafe_allow_html=True)

        col1, col2 = st.columns(2)
        with col1:
            applicant_name = st.text_input("Applicant Name", placeholder="e.g., James Richardson")
        with col2:
            applicant_id = st.text_input("Applicant ID", placeholder="e.g., FAIR_001")

        col1, col2, col3 = st.columns(3)
        with col1:
            applicant_age = st.number_input("Age", min_value=18, max_value=120, value=None, placeholder=None)
        with col2:
            applicant_race = st.selectbox("Race", ["", "White", "Hispanic", "African American", "Asian", "Other"], key="race")
        with col3:
            applicant_gender = st.selectbox("Gender", ["", "Male", "Female", "Other"], key="gender")

        col1, col2 = st.columns(2)
        with col1:
            applicant_income = st.number_input("Annual Income ($)", min_value=0, value=None, placeholder=None)
        with col2:
            applicant_credit = st.number_input("Credit Score", min_value=300, max_value=850, value=None, placeholder=None)

        applicant_denied = st.checkbox("Application Denied?")
        if applicant_denied:
            applicant_denial_reason = st.text_input("Denial Reason", placeholder="e.g., credit_score_too_low")
        else:
            applicant_denial_reason = None

        # ── Date Range Section ──────────────────────────────────────────────────
        st.markdown('<div style="margin:16px 0 8px; font-size:13px; font-weight:700; color:#475569;">Date Range</div>', unsafe_allow_html=True)
...
with right_col:
    ...
    if submitted and query.strip():
        ...
        try:
            # Build applicant data if provided
            applicant_data = None
            if applicant_name or applicant_income or applicant_credit:
                applicant_data = {}
                if applicant_name:
                ...
                if applicant_age:
                    applicant_data["age"] = int(applicant_age)
                if applicant_race and applicant_race != "":
                    applicant_data["race"] = applicant_race
                if applicant_gender and applicant_gender != "":
                    applicant_data["gender"] = applicant_gender
                if applicant_income:
                    applicant_data["income"] = int(applicant_income)
                if applicant_credit:
                    applicant_data["credit_score"] = int(applicant_credit)
                applicant_data["denied"] = applicant_denied
                if applicant_denied and applicant_denial_reason:
                    applicant_data["denial_reason"] = applicant_denial_reason
            ...
            if r.status_code == 202:
                ...
                # Trigger synchronous execution (fallback if Redis not available)
                try:
                    ...
                    if exec_resp.status_code != 200:
                        st.warning(f"Sync execution returned {exec_resp.status_code}")
                except Exception as exec_err:
                    st.warning(f"Could not trigger sync execution: {exec_err}")
            else:
...
""", unsafe_allow_html=True)
    ...
    # ── Active investigation ────────────────────────────────────────────────────
        ...
        _PROG = {
            ...
            "analyzing": 58, "assembling": 78, "reporting": 91, "auditing": 97,
        ...
        STATUS_LABELS = {
            ...
            "auditing":      "Writing audit log entries",
            "complete":      "Complete",
...
""", unsafe_allow_html=True)
                ...
                grid = (
                    ...
                    f'<div style="margin-bottom:8px;">{_node("report_agent")}</div>'
                    f'<div>{_node("audit_agent")}</div>'
                )
